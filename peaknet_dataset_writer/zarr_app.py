...
from .zarr_writer import write_results_to_zarr
...
def process_event_batch(batch_idx, event_batch, max_concurrent_subtasks, shared_data_ref):
    ...
    for event in event_batch:
        ...
        # Skip to the next event if no good peaks are found
        if len(good_peaks) == 0:
            continue
    ...
    file_h5 = f"{basename_h5}.{batch_idx:04d}.zarr"
    path_h5 = os.path.join(dir_h5, file_h5)
    write_results_to_zarr(path_h5, results)
